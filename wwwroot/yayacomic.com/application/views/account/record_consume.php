...
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<title>消费记录 - <?=SITE()["name"]?></title>
		<meta name="keywords" content="消费记录,<?=SITE()["keywords"]?>" />
		<meta name="Author" content="<?=SITE()["author"]?>" />
		<meta name="Description" content="消费记录,<?=SITE()["description"]?>" />
	...
	<body class="bg-gray">
		<?php $this->load->view("layout/header", array("title"=>"消费记录"))?>
		...
		<div id="wrapper">
			<ul class="vip-record">
				<li class="gary">
					<p class="vip-record-title">山海逆战</p>
					<p class="vip-record-tip">2019-05-23 17:03</p>
					<span class="vip-record-right">(福币)&nbsp;&nbsp;-89</span>
				</li>
			</ul>
		</div>
		...
		<script type="text/javascript">if (document.getElementById('scroller') && document.getElementById('scroller').offsetHeight + 60 >= document.getElementById('wrapper').clientHeight) {
			...
			var timestamp = Number('1558602203577' || '0');
			var type = Number('2');
