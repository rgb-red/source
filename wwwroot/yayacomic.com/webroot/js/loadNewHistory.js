...
/*window.onscroll = function () {
    ...
    if (($("body").height() - scrollTop) <= document.documentElement.clientHeight && loadSign == 0 ) {
        ...
        if(loadPage != 0){
            ...
            //获取数据
            $.ajax({
                url: 'pagerdataAAA.ashx?d=' + new Date(),
                dataType: 'json',
                ...
                success: function (data) {
                    if(data.length > 0){
                        ...
                        for(var n = 0;n < data.length;n++){
                            ...
                            result += '<a href="/' + data[n].MangaUrl + '/?from=' + CURRENTURL + '"  class="readlink" mid="' + data[n].Mid + '"><p class="buy-manga-title">' + data[n].MName + '</p></a>';
...
function clearReadhistory() {
    var userid = USERID;
    $.ajax({
        url: "/ajax/bookHistory_post?t=" + new Date().getTime(), 
        dataType: 'json',
        data: { type: "clearall" }, 
        type: 'POST',
        success: function (data) {
