...
$(function () {
    ...
    $('.search-downlist-item').click(function (e) {
        ...
        if (item && $.trim($(item).text()) !== '') {
            location.href = "/home/search?title=" + encodeURIComponent($.trim($(item).text()));
        }
    ...
    $('.search-downlist-item-cross').click(function (e) {
        ...
        $.get("/home/rmSearchHistory?action=removehistory&t=" + new Date().getTime(), { title: encodeURIComponent(title) }, function (data) {
            if (data && data["result"]) {
        ...
        $(this).parent(".search-downlist-item").hide();
    });
});
//右侧悬浮返回顶部
...
function doSearch(isreplace) {
    ...
    if ($.trim(keywords) != "") {
        if (isreplace) {
            pushHistory("/home/search?title=" + encodeURIComponent(keywords));
        } else {
            location.href = "/home/search?title=" + encodeURIComponent(keywords);
        }
    }
}
//重新搜索
...
function historyClear() {
    $.get("/home/rmSearchHistory?action=removeall", function () {
        $(".search-downlist").remove();
    });
}
//登录验证
...
function setBookmarker(cid, mid, p) {
    if (canBookmarker) {
        ...
        if ($("#collect_icon").attr("src").indexOf("_a") == -1) {
            $.ajax({
                url: 'bookmarkeraaa.ashx?d=' + new Date().getTime(),
                dataType: 'json',
            ...
            $.ajax({
                url: 'bookmarkerbbb.ashx?d=' + new Date().getTime(),
                dataType: 'json',
...
function getMoreComments() {
    loadPage++;
    $.ajax({
        url: "pagerdataDDD.ashx?d=" + new Date().getTime(),
        dataType: "json",
        data: {
...
function getFirstChapter(url, mid) {
    $.ajax({
        url: '/pagerdataEEE.ashx?d=' + new Date().getTime(),
        dataType: 'text',
        data: { t: 3, mid: mid },
        type: 'POST',
        success: function (data) {
...
function validcode(tid, code, action) {
    ...
    $.ajax({
        url: '/checkcodeaaa.ashx?d=' + new Date().getTime(),
        dataType: 'json',
